...
<head>
    ...
    <title>AMPDB Data Stat by Hydrophobicity (GRAVY)</title>
...
<body>
    ...
    <div class="container mt-3">
        <div class="row" style="padding-left: 0rem;">
            <div class="col-sm text-start" style="font-weight: 400; font-size:xx-large">
                AMPDB Statistics: AMPDB Data Distribution by Hydrophobicity (GRAVY)
            </div>
        </div>
    </div>
                    ...
                    <table class="table border border-2 table-striped table-hover">
                        <thead class="bg-dark text-white">
                            <tr>
                                ...
                                <th style="font-weight: 500;">Range of Hydrophobicity (GRAVY)</th>
    ...
    <script>
        ...
        function rescaleYAxis() {
            var newMax = Math.floor(Math.random() * 10000) + 1000;
            myChart.options.scales.y.max = newMax;
            myChart.update();
        }

        function rescaleXAxis() {
            var newMax = Math.floor(Math.random() * 16);
        ...
        myChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['-3.4 >= to < -1.0', '-1.0 >= to < -0.9', '-0.9 >= to < -0.8', '-0.8 >= to < -0.7', '-0.7 >= to < -0.6', '-0.6 >= to < -0.5', '-0.5 >= to < -0.4', '-0.4 >= to < -0.3', '-0.3 >= to < -0.2', '-0.2 >= to < -0.1', '-0.1 >= to < 0.0', '0.0 >= to < 0.1', '0.1 >= to < 0.2', '0.2 >= to < 0.3', '0.3 >= to < 0.4', '0.4 >= to < 2.8'],
                datasets: [{
                    label: '# of Record(s)',
                    data: [274, 217, 640, 1875, 3597, 4697, 7421, 8769, 7577, 6052, 4813, 4315, 3136, 1967, 1200, 2572],
