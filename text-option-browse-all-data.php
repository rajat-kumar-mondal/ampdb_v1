...
<hr style="height: 3px;">
<?php
$sql = "SELECT `peptide activity` FROM `master2` WHERE (AMPDB_No_ LIKE '$term' 
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `target organism` FROM `master2` WHERE (AMPDB_No_ LIKE '$term' 
OR Reviewed LIKE '%$term%'
OR `Entry` LIKE '%$term%'
OR Organism LIKE '%$term%'
OR `Taxonomic lineage` LIKE '%$term%'
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `Organism` FROM `master2` WHERE (AMPDB_No_ LIKE '$term' 
OR Reviewed LIKE '%$term%'
OR `Entry` LIKE '%$term%'
OR Organism LIKE '%$term%'
OR `Taxonomic lineage` LIKE '%$term%'
...
$targetArrString = implode(" ", $targetArr);
$mysql = "SELECT DISTINCT(`Organism`) FROM `master2` WHERE (AMPDB_No_ LIKE '$term' 
OR Reviewed LIKE '%$term%'
OR `Entry` LIKE '%$term%'
OR Organism LIKE '%$term%'
OR `Taxonomic lineage` LIKE '%$term%'
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `Protein existence` FROM `master2` WHERE (AMPDB_No_ LIKE '$term' 
OR Reviewed LIKE '%$term%'
OR `Entry` LIKE '%$term%'
OR Organism LIKE '%$term%'
OR `Taxonomic lineage` LIKE '%$term%'
...
for ($i = 0; $i < 6; $i += 1) {
    ...
    $mysql5 = "SELECT COUNT(Serial_No) FROM `master2` WHERE (AMPDB_No_ LIKE '$term' 
    OR Reviewed LIKE '%$term%'
    OR `Entry` LIKE '%$term%'
    OR Organism LIKE '%$term%'
    OR `Taxonomic lineage` LIKE '%$term%'
    ...
    OR `activity` LIKE '%$term%'
    OR `Protein existence` LIKE '%$term%'
    OR `target organism` LIKE '%$term%') $selDs $filterQuery AND $myval $sortFilter";
