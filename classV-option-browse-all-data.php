...
<hr style="height: 3px;">
<?php
$sql = "SELECT `peptide activity` FROM `master2` WHERE (`activity` LIKE '%$term%') $selDs $filterQuery $sortFilter";
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `target organism` FROM `master2` WHERE (`activity` LIKE '%$term%') $selDs $filterQuery $sortFilter";
$result = $con->query($sql);
$targetArr = array();
while ($row = $result->fetch_assoc()) {
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `Organism` FROM `master2` WHERE (`activity` LIKE '%$term%') $selDs $filterQuery $sortFilter";
$result = $con->query($sql);
$targetArr = array();
while ($row = $result->fetch_assoc()) {
...
$targetArrString = implode(" ", $targetArr);
$mysql = "SELECT DISTINCT(`Organism`) FROM `master2` WHERE (`activity` LIKE '%$term%') $selDs $filterQuery $sortFilter";
$targetList = array();
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `Protein existence` FROM `master2` WHERE (`activity` LIKE '%$term%') $selDs $filterQuery $sortFilter";
$result = $con->query($sql);
$targetArr = array();
while ($row = $result->fetch_assoc()) {
...
for ($i = 0; $i < 6; $i += 1) {
    ...
    $mysql5 = "SELECT COUNT(Serial_No) FROM `master2` WHERE (`activity` LIKE '%$term%') $selDs $filterQuery AND $myval $sortFilter";
