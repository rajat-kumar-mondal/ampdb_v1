...
<hr style="height: 3px;">
<?php
$sql = "SELECT `peptide activity` FROM `master` WHERE (AMPDB_No_ LIKE '$term' 
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `target organism` FROM `master` WHERE (AMPDB_No_ LIKE '$term' 
OR Reviewed LIKE '%$term%'
OR `Entry` LIKE '%$term%'
OR Organism LIKE '%$term%'
OR `Taxonomic lineage` LIKE '%$term%'
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `Organism` FROM `master` WHERE (AMPDB_No_ LIKE '$term' 
OR Reviewed LIKE '%$term%'
OR `Entry` LIKE '%$term%'
OR Organism LIKE '%$term%'
OR `Taxonomic lineage` LIKE '%$term%'
...
$targetArrString = implode(" ", $targetArr);
$mysql = "SELECT DISTINCT(`Organism`) FROM `master` WHERE (AMPDB_No_ LIKE '$term' 
OR Reviewed LIKE '%$term%'
OR `Entry` LIKE '%$term%'
OR Organism LIKE '%$term%'
OR `Taxonomic lineage` LIKE '%$term%'
...
<hr style="height: 3px;">
<?php
$sql = "SELECT `Protein existence` FROM `master` WHERE (AMPDB_No_ LIKE '$term' 
OR Reviewed LIKE '%$term%'
OR `Entry` LIKE '%$term%'
OR Organism LIKE '%$term%'
OR `Taxonomic lineage` LIKE '%$term%'
...
for ($i = 0; $i < 6; $i += 1) {
    ...
    $mysql5 = "SELECT COUNT(Serial_No) FROM `master` WHERE (AMPDB_No_ LIKE '$term' 
    OR Reviewed LIKE '%$term%'
    OR `Entry` LIKE '%$term%'
    OR Organism LIKE '%$term%'
    OR `Taxonomic lineage` LIKE '%$term%'
