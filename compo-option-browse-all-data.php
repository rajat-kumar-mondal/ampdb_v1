...
<hr style="height: 3px;">
<?php
$sql = str_replace(" undefined", "", "SELECT `peptide activity` FROM `master2` WHERE ($term) $selDs $filterQuery $sortFilter");
...
<hr style="height: 3px;">
<?php
$sql = str_replace(" undefined", "", "SELECT `target organism` FROM `master2` WHERE ($term) $selDs $filterQuery $sortFilter");
$result = $con->query($sql);
$targetArr = array();
while ($row = $result->fetch_assoc()) {
...
<hr style="height: 3px;">
<?php
$sql = str_replace(" undefined", "", "SELECT `Organism` FROM `master2` WHERE ($term) $selDs $filterQuery $sortFilter");
$result = $con->query($sql);
$targetArr = array();
while ($row = $result->fetch_assoc()) {
...
$targetArrString = implode(" ", $targetArr);
$mysql = str_replace(" undefined", "", "SELECT DISTINCT(`Organism`) FROM `master2` WHERE ($term) $selDs $filterQuery $sortFilter");
$targetList = array();
...
<hr style="height: 3px;">
<?php
$sql = str_replace(" undefined", "", "SELECT `Protein existence` FROM `master2` WHERE ($term) $selDs $filterQuery $sortFilter");
$result = $con->query($sql);
$targetArr = array();
while ($row = $result->fetch_assoc()) {
...
for ($i = 0; $i < 6; $i += 1) {
    ...
    $mysql5 = str_replace(" undefined", "", "SELECT COUNT(Serial_No) FROM `master2` WHERE ($term) $selDs $filterQuery AND $myval $sortFilter");
